...
async function rProfile(){
  ...
  if(na)na.innerHTML=A.user?`<span style="font-size:12px;font-weight:600;color:#122A4A">${A.user.name||"User"}</span><button class="btn btn-ghost btn-sm" onclick="logout()">Logout</button>`:`<button class="btn btn-outline btn-sm" onclick="go('patientLogin')">Login to Book</button>`;
  ...
  if($("profile-content"))$("profile-content").innerHTML=`
    <div class="profile-header">
      <div style="display:flex;gap:18px;margin-bottom:18px;flex-wrap:wrap">
        ${avt(d.name,d.color||"#0F3D73",74)}
        <div style="flex:1;min-width:160px">
          <h2 style="font-size:clamp(18px,4vw,24px);font-weight:800;color:#122A4A;margin-bottom:3px">${safe(d.name)}</h2>
          <div style="font-size:14px;color:#0F3D73;font-weight:700;margin-bottom:8px">${safe(d.specialization)}</div>
          <div style="display:flex;gap:6px;flex-wrap:wrap">
            ...
            ${d.approved?pill("✓ Verified","#D1FAE5","#1C8F7D"):""}
            ${d.video_consult?pill("🎥 Video","#EDE9FE","#5B4FCF"):""}
          </div>
        </div>
        <div style="text-align:right"><div style="font-size:16px;font-weight:700;color:#1C8F7D">🆓 Free Booking</div></div>
      </div>
      ...
      ${d.is_available===false?`<div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:10px;padding:10px 14px;margin-bottom:10px;font-size:12px;color:#B23434;font-weight:600">🔴 Doctor is currently offline — booking unavailable</div>`:""}
...
async function rBook(){
  ...
  if(!A.user||A.role!=="patient"){c.innerHTML=`<div style="text-align:center;padding:40px 0"><div style="font-size:56px;margin-bottom:14px">🔒</div><h3 style="font-family:'Sora',serif;font-size:22px;color:#122A4A;margin-bottom:8px">Login Required</h3><p style="color:#6B7280;font-size:14px;margin-bottom:20px">Book చేయాలంటే ముందు Patient గా Login చేయండి</p><button class="btn btn-primary" onclick="go('patientLogin')">Patient Login</button></div>`;return;}
  A.slot="";
  ...
  const stepNum=(n)=>`<span style="display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:#0F3D73;color:#fff;font-size:12px;font-weight:700;flex-shrink:0">${n}</span>`;
  c.innerHTML=`
    <div style="background:#fff;border-radius:14px;padding:14px;margin-bottom:14px;display:flex;gap:12px;box-shadow:0 2px 8px rgba(0,0,0,.05)">
      ${avt(d.name,d.color||"#0F3D73",48)}
      <div style="flex:1"><div style="font-size:15px;font-weight:700;color:#122A4A">${safe(d.name)}</div>
      <div style="font-size:12px;color:#0F3D73;font-weight:600">${safe(d.specialization)}</div>
      <div style="font-size:12px;color:#9CA3AF">🏥 ${safe(d.hospital)}</div></div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
        <div style="font-size:16px;font-weight:700;color:#1C8F7D">${d.fee?`₹${d.fee}`:"🆓 Free"}</div>
    ...
    ${d.video_consult?`<div style="background:#fff;border-radius:14px;padding:14px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.05)">
      <div class="form-label" style="display:flex;align-items:center;gap:8px;margin-bottom:10px">${stepNum(1)} ఎలా Consult అవ్వాలి?</div>
    ...
    <div style="background:#fff;border-radius:16px;padding:20px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.05)">
      <div class="form-group">
        <label class="form-label" style="display:flex;align-items:center;gap:8px;margin-bottom:8px">${stepNum(d.video_consult?2:1)} ఏ రోజు రావాలి? <span>*</span></label>
        <input type="date" id="book-date" class="form-input" min="${td()}" onchange="onDateChange()"/>
      </div>
      <div class="form-group" id="slot-step" style="opacity:.4;pointer-events:none;transition:opacity .25s">
        <label class="form-label" style="display:flex;align-items:center;gap:8px;margin-bottom:8px">${stepNum(d.video_consult?3:2)} ఏ సమయం కావాలి? <span>*</span></label>
        <div style="font-size:12px;color:#9CA3AF;margin-bottom:10px" id="slot-hint">☝️ ముందు తేదీ ఎంచుకోండి</div>
      ...
      <div id="book-summary" style="display:none;background:#F0FDF4;border-radius:10px;padding:14px;border:1px solid #A7F3D0;margin-top:12px">
        ...
        <div style="font-size:15px;font-weight:700;color:#1C8F7D;margin-top:6px">${d.fee?`₹${d.fee}`:"🆓 Free Booking"}</div>
      </div>
    </div>
    <button class="btn btn-primary btn-full" id="confirm-btn" onclick="confirmBook()" style="font-size:15px;padding:14px" disabled>ముందు తేదీ & సమయం ఎంచుకోండి</button>`;
}
function unlockSlotStep(){
  const step=$("slot-step"),hint=$("slot-hint");
  if(step){step.style.opacity="1";step.style.pointerEvents="auto";}
...
function updateBookSum(){
  const date=($("book-date")||{}).value,sb=$("book-summary"),st=$("bsum"),cb=$("confirm-btn"),fee=A.doc?.fee;
  const ready=!!(date&&A.slot);
  if(sb&&st&&ready){sb.style.display="block";st.innerHTML=`📅 ${date}<br>🕐 ${A.slot}<br>👤 ${A.user.name}<br>${A.type==="video"?"🎥 Video Consultation":"🏥 In-Person"}`;}else if(sb)sb.style.display="none";
  if(cb){
    cb.disabled=!ready;
    cb.textContent=ready?("Confirm Booking — "+(fee?"₹"+fee:"Free")):(date?"ముందు సమయం ఎంచుకోండి":"ముందు తేదీ & సమయం ఎంచుకోండి");
  }
}
async function confirmBook(){
  ...
  finally{if(btn){btn.disabled=false;btn.textContent="Confirm Booking — "+(A.doc?.fee?"₹"+A.doc.fee:"Free");}}
}
...
function openLiveTracking(type, name, phone, lat, lng){
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:24px;max-width:360px;width:100%">
      <div style="font-size:18px;font-weight:700;color:#122A4A;margin-bottom:6px">
...
function storeCard(s){
  ...
  return `<div class="card" onclick="A.user?viewStore('${s.id}'):requireLogin('stores')">
    <div style="display:flex;gap:12px;margin-bottom:10px">
      <div style="width:52px;height:52px;border-radius:12px;background:${s.color||"#22A490"};display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0">💊</div>
      <div style="flex:1;min-width:0">
        <div class="doc-name">${safe(s.store_name)}</div>
        <div style="font-size:12px;color:#6B7280">👤 ${safe(s.owner_name)}</div>
        <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px">
          ${pill(isOpen?"🟢 Open":"🔴 Closed",isOpen?"#D1FAE5":"#FEE2E2",isOpen?"#1C8F7D":"#B23434")}
          ${s.delivery_available?pill("🚴 Delivery","#E3F4FC","#0F3D73"):pill("Pickup Only","#F3F4F6","#9CA3AF")}
          ${s.is_24x7?pill("24×7","#EDE9FE","#5B4FCF"):""}
        </div>
      </div>
    </div>
...
function rStoreDetail(){
  ...
  if($("store-detail-content"))$("store-detail-content").innerHTML=`<div class="profile-header">
    <div style="display:flex;gap:16px;margin-bottom:16px;flex-wrap:wrap">
      <div style="width:64px;height:64px;border-radius:16px;background:${s.color||"#22A490"};display:flex;align-items:center;justify-content:center;font-size:28px;flex-shrink:0">💊</div>
      <div style="flex:1;min-width:160px"><h2 style="font-size:20px;font-weight:800;color:#122A4A;margin-bottom:3px">${safe(s.store_name)}</h2><div style="font-size:13px;color:#6B7280">👤 ${safe(s.owner_name)}</div>
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">${pill(s.is_available!==false?"🟢 Open":"🔴 Closed",s.is_available!==false?"#D1FAE5":"#FEE2E2",s.is_available!==false?"#1C8F7D":"#B23434")}${s.approved?pill("✓ Verified","#D1FAE5","#1C8F7D"):""}${s.is_24x7?pill("24×7","#EDE9FE","#5B4FCF"):""}${s.delivery_available?pill("🚴 Delivery","#E3F4FC","#0F3D73"):""}</div>
...
async function rPrescOrder(){
  ...
  c.innerHTML=`<div style="background:#fff;border-radius:16px;padding:20px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.05)">
    ${A.store?`<div style="display:flex;gap:12px;align-items:center;padding:12px;background:#F0FDF4;border-radius:12px;margin-bottom:16px"><span style="font-size:24px">💊</span><div><div style="font-size:14px;font-weight:700;color:#122A4A">${safe(A.store.store_name)}</div><div style="font-size:12px;color:#1C8F7D">Sending to this store</div></div><button class="btn btn-ghost btn-sm" style="margin-left:auto" onclick="A.store=null;rPrescOrder()">Change</button></div>`:
    ...
    <div class="form-group"><label class="form-label">Doctor's Prescription Photo <span>*</span></label>
      <div class="upload-area" id="presc-area" onclick="$('presc-file').click()"><div style="font-size:40px;margin-bottom:8px">📸</div><div style="font-size:14px;font-weight:600;color:#0F3D73">Click to upload prescription</div><div style="font-size:12px;color:#9CA3AF;margin-top:4px">JPG, PNG, PDF · Max 5MB</div></div>
...
function onPrescFile(e){
  ...
  else{prev.innerHTML=`<div style="padding:10px;background:#F0FDF4;border-radius:8px;color:#1C8F7D">📄 ${file.name}</div>`;prev.style.display="block";}
  const a=$("presc-area");if(a){a.style.borderColor="#22A490";a.innerHTML=`<div style="font-size:30px;margin-bottom:6px">✅</div><div style="font-size:13px;font-weight:600;color:#1C8F7D">${file.name}</div>`;}
}
...
async function rPatientOrders(){
  ...
  try{
    ...
    const sc2={pending:"#D97706",accepted:"#0F3D73",packed:"#5B4FCF",delivered:"#1C8F7D",completed:"#1C8F7D",rejected:"#B23434"};
...
async function rAmbulance(){
  ...
  try{
    ...
    listEl.innerHTML=ambus.map(a=>`<div class="card">
      <div style="display:flex;gap:12px;margin-bottom:10px">
        <div style="width:52px;height:52px;border-radius:12px;background:${a.color||"#D64545"};display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0">🚑</div>
        <div style="flex:1;min-width:0">
          <div style="font-size:15px;font-weight:700;color:#122A4A">${safe(a.operator_name)}</div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:12px">
        <div style="background:#FEF2F2;border-radius:8px;padding:8px;text-align:center"><div style="font-size:10px;color:#9CA3AF;margin-bottom:1px">Base Fare</div><div style="font-size:13px;font-weight:700;color:#B23434">₹${a.base_fare||0}</div></div>
        <div style="background:#FEF2F2;border-radius:8px;padding:8px;text-align:center"><div style="font-size:10px;color:#9CA3AF;margin-bottom:1px">Per KM</div><div style="font-size:13px;font-weight:700;color:#B23434">₹${a.per_km_rate||0}</div></div>
        <div style="background:#FEF2F2;border-radius:8px;padding:8px;text-align:center"><div style="font-size:10px;color:#9CA3AF;margin-bottom:1px">Radius</div><div style="font-size:13px;font-weight:700;color:#B23434">${a.service_radius_km||0}km</div></div>
      </div>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px">
        ${a.has_oxygen?`<span style="font-size:11px;background:#F0FDF4;color:#1C8F7D;padding:3px 8px;border-radius:8px">🩸 Oxygen</span>`:""}
        ${a.has_stretcher?`<span style="font-size:11px;background:#F0FDF4;color:#1C8F7D;padding:3px 8px;border-radius:8px">🛏️ Stretcher</span>`:""}
        ${a.has_monitor?`<span style="font-size:11px;background:#F0FDF4;color:#1C8F7D;padding:3px 8px;border-radius:8px">📊 Monitor</span>`:""}
      </div>
...
function labCard(l){
  ...
  return `<div class="card" onclick="A.user?viewLab('${l.id}'):requireLogin('labsPage')">
    <div style="display:flex;gap:12px;margin-bottom:10px">
      <div style="width:52px;height:52px;border-radius:12px;background:${l.color||"#5B4FCF"};display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0">${ti[l.lab_type]||"🔬"}</div>
      <div style="flex:1;min-width:0">
        <div class="doc-name">${safe(l.lab_name)}</div>
        <div style="font-size:12px;color:#5B4FCF;font-weight:600">${(l.lab_type||"").replace("_"," ").toUpperCase()}</div>
        <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px">
          ${pill(l.is_available!==false?"🟢 Open":"🔴 Closed",l.is_available!==false?"#D1FAE5":"#FEE2E2",l.is_available!==false?"#1C8F7D":"#B23434")}
          ${l.nabl_accredited?pill("NABL","#D1FAE5","#1C8F7D"):""}
          ${l.home_collection?pill("🏠 Home","#EDE9FE","#5B4FCF"):""}
...
async function rLabDetail(){
  ...
  const cc={Blood:"#FF4757",Urine:"#FFC22E",Radiology:"#2196F3",Package:"#009688",Special:"#F59E0B"};
  c.innerHTML=`<div class="profile-header">
    <div style="display:flex;gap:16px;margin-bottom:16px;flex-wrap:wrap">
      <div style="width:64px;height:64px;border-radius:16px;background:${l.color||"#5B4FCF"};display:flex;align-items:center;justify-content:center;font-size:28px;flex-shrink:0">🔬</div>
      <div style="flex:1;min-width:160px"><h2 style="font-size:20px;font-weight:800;color:#122A4A;margin-bottom:3px">${safe(l.lab_name)}</h2>
        <div style="font-size:13px;color:#5B4FCF;font-weight:600">${(l.lab_type||"").replace("_"," ").toUpperCase()}</div>
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">${l.nabl_accredited?pill("✓ NABL","#D1FAE5","#1C8F7D"):""}${l.home_collection?pill("🏠 Home Collection","#EDE9FE","#5B4FCF"):""}</div>
      </div>
    </div>
  ...
  <div style="font-size:16px;font-weight:700;color:#122A4A;margin-bottom:14px">Available Tests (${tests.length})</div>
  ${Object.entries(groups).map(([cat,ct])=>`<div style="background:#fff;border-radius:14px;margin-bottom:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.05)">
    <div style="padding:12px 16px;background:${cc[cat]||"#0F3D73"}18;font-size:13px;font-weight:700;color:${cc[cat]||"#0F3D73"}">${cat} (${ct.length})</div>
    ${ct.map(t=>`<div style="padding:12px 16px;border-bottom:1px solid #F3F4F6;display:flex;align-items:center;gap:10px">
      <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:600;color:#122A4A">${A.lang==="te"&&t.test_name_te?t.test_name_te:t.test_name}</div>
        <div style="display:flex;gap:6px;margin-top:3px;flex-wrap:wrap">
          ...
          ${t.home_available?`<span style="font-size:10px;color:#5B4FCF;background:#F5F3FF;padding:1px 6px;border-radius:4px">Home</span>`:""}
          <span style="font-size:10px;color:#6B7280">⏱ ${t.report_time||"Same Day"}</span>
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:600;color:#5B4FCF">₹${t.price||0}</div>
        ${t.discount>0?`<div style="font-size:10px;color:#1C8F7D;font-weight:600">${t.discount}% off</div>`:""}
    ...
    if(bc&&banners.length){
      ...
      const renderBanner=()=>{
        ...
        bc.innerHTML=banner.image_url?
          ...
          `<div style="background:linear-gradient(135deg,#0F3D73,#0A2C54);border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:12px;cursor:pointer;margin-bottom:16px;transition:opacity 0.3s" onclick="${banner.link_url?`window.open('${banner.link_url}','_blank')`:''}">
      ...
      <div style="margin-bottom:16px">
        ...
        ${grouped[date].map(r=>`<div class="card" style="margin-bottom:8px">
          <div style="display:flex;gap:12px;align-items:center">
            ...
            <div style="flex:1">
              <div style="font-size:13px;font-weight:600;color:#122A4A">${safe(r.file_name)}</div>
