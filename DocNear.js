    ...
    safeGet("ambulances","approved=eq.true&availability_status=eq.available&limit=3","ambus").then(ambus=>{
      ...
      amc.innerHTML=ambus.length?ambus.map(a=>`<div class="card" onclick="go('ambulance')">
        ...
        <div style="display:flex;gap:6px">
          <button class="btn btn-outline btn-sm" onclick="event.stopPropagation();shareProfile('ambulance','${a.id}','${safe(a.operator_name)}','${safe(a.vehicle_type||'Ambulance')}','${safe(a.district||'')}')" >📤</button>
          <a href="tel:${safe(a.phone)}" class="btn btn-primary" style="flex:1;font-size:12px;padding:7px;background:#EF4444;border-color:#EF4444;text-align:center" onclick="event.stopPropagation()">📞 Call Now</a>
        </div>
  ...
  if(lbc){
    ...
    safeGet("diagnostic_labs","approved=eq.true&limit=3","labs").then(labs=>{
      ...
      lbc.innerHTML=labs.length?labs.map(l=>`<div class="card" onclick="go('labsPage')">
        ...
        <div style="display:flex;gap:6px">
          <button class="btn btn-outline btn-sm" onclick="event.stopPropagation();shareProfile('lab','${l.id}','${safe(l.lab_name||l.name)}','Diagnostic Lab','${safe(l.district||'')}')" >📤</button>
...
function docCard(d){
  ...
  return `<div class="card" onclick="viewDoc('${d.id}')">
    ...
    <div style="display:flex;gap:8px">
      ...
      <button class="btn btn-outline btn-sm" onclick="event.stopPropagation();shareProfile('doctor','${d.id}','${safe(d.name)}','${safe(d.specialization)}','${safe(d.district||'')}')" title="Share">📤</button>
...
function storeCard(s){
  ...
  return `<div class="card" onclick="viewStore('${s.id}')">
    ...
    <div style="display:flex;gap:8px">
      ...
      <button class="btn btn-outline btn-sm" onclick="event.stopPropagation();shareProfile('store','${s.id}','${safe(s.store_name||s.name)}','Medical Store','${safe(s.district||s.city||'')}')" >📤</button>
...
function labCard(l){
  ...
  return `<div class="card" onclick="viewLab('${l.id}')">
    ...
    <div style="display:flex;gap:8px">
      ...
      <button class="btn btn-outline btn-sm" onclick="event.stopPropagation();shareProfile('lab','${l.id}','${safe(l.lab_name)}','${safe((l.lab_type||'').replace('_',' '))}','${safe(l.district||'')}')" title="Share">📤</button>
...
function hpCard(p){
  ...
  return `<div class="card" style="margin-bottom:12px">
    ...
    <div style="display:flex;gap:8px">
      ...
      <button class="btn btn-primary" style="flex:1" onclick="selectHSProvider('${p.id}','${safe(p.name)}','${safe(p.service_type)}')">📅 Book</button>
    </div>
  </div>`;
}
