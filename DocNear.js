  ...
  try{
    ...
    if(bc&&banners.length){
      ...
      const renderBanner=()=>{
        ...
        bc.innerHTML=`<div style="background:linear-gradient(135deg,#0A7FAF,#074E7A);border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:12px;cursor:pointer;margin-bottom:16px;transition:opacity 0.3s" onclick="${banner.link_url?`window.open('${banner.link_url}','_blank')`:''}">
          ${isVid?`<video src="${banner.image_url}" autoplay muted loop style="width:60px;height:60px;border-radius:8px;object-fit:cover"></video>`:
