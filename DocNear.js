...
async function rLanding(){
  ...
  if(stc){
    spin("landing-stores");
    try{
      let stores=await safeGet("medical_stores","approved=eq.true&limit=3","stores");
  ...
  if(amc){
    spin("landing-ambu");
    try{
      let ambus=await safeGet("ambulances","approved=eq.true&availability_status=eq.available&limit=3","ambus");
  ...
  if(lbc){
    spin("landing-labs");
    try{
      let labs=await safeGet("diagnostic_labs","approved=eq.true&limit=3","labs");
