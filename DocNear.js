...
async function toggleDoctorAvailability(){
  ...
  const next=!cur;
  try{
    await DB.patch("doctors","id=eq."+A.user.id,{is_available:next}).catch(()=>{});
...
async function toggleStoreAvailability(){
  ...
  const next=!cur;
  try{
    await DB.patch("medical_stores","id=eq."+A.user.id,{is_available:next}).catch(()=>{});
...
async function toggleLabAvailability(){
  ...
  const next=!cur;
  try{
    await DB.patch("diagnostic_labs","id=eq."+A.user.id,{is_available:next}).catch(()=>{});
...
async function loadAds(){
  try{
    ...
    if(splash){
      ...
      div.innerHTML=`<div style="position:absolute;top:16px;right:16px;background:rgba(255,255,255,0.2);color:#fff;padding:6px 14px;border-radius:20px;font-size:13px;font-weight:600" id="ad-timer">Skip in 10s</div>
    ...
    // Banner ad
    const banner=ads.find(a=>a.ad_type==="banner");
    const bc=document.getElementById("home-banner-ad");
    if(bc&&banner){
      bc.style.display="block";
      bc.innerHTML=`<div style="background:linear-gradient(135deg,#0A7FAF,#074E7A);border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:12px;cursor:pointer;margin-bottom:16px" onclick="${banner.link_url?`window.open('${banner.link_url}','_blank')`:''}">
        ${banner.image_url?`<img src="${banner.image_url}" style="width:60px;height:60px;border-radius:8px;object-fit:cover"/>`:
        ...
        <div style="color:rgba(255,255,255,0.8);font-size:11px;margin-top:2px">Sponsored</div></div>
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  if(sess&&!fromAdmin){A.user=sess.u;A.role=sess.r;redirectRole(sess.r);}
