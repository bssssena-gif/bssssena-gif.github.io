...
function go(pg){
  ...
  const map={
    ...
    hpLogin:()=>{},hpReg:()=>{},hpPending:()=>{},hpDash:rHPDash
  };
  if(map[pg])map[pg]();
}
function logout(){
  ...
  try{
    ...
    if(!providers.length){
      el.innerHTML=`<div style="text-align:center;padding:20px;background:#F8FAFC;border-radius:12px;color:#9CA3AF">
        ...
        <div>మీ జిల్లాలో ప్రస్తుతం providers అందుబాటులో లేరు.</div>
      </div>`;return;
    }
...
function selectHSProvider(id,name,type){
  // Pre-select provider & scroll to form
  ...
  if(sel){
    ...
    sel.innerHTML=`<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:10px;padding:12px;margin-bottom:12px;display:flex;align-items:center;gap:10px">
      <div style="font-size:28px">👩‍⚕️</div>
      <div><div style="font-weight:700;color:#1A2B3C">${name}</div><div style="font-size:12px;color:#0891B2">Selected Provider</div></div>
    </div>`;
  }
  // Store selected provider id as hidden input
  let inp=$("hs-provider-id");
  if(!inp){inp=document.createElement("input");inp.type="hidden";inp.id="hs-provider-id";const form=$("hs-name");if(form&&form.parentNode)form.parentNode.appendChild(inp);}
  inp.value=id;
  // Scroll to booking form
  const form=$("hs-name");if(form)form.scrollIntoView({behavior:"smooth",block:"start"});
