  ...
  ${Object.entries(groups).map(([cat,ct])=>`<div style="background:#fff;border-radius:14px;margin-bottom:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.05)">
    ...
    ${ct.map(t=>`<div style="padding:12px 16px;border-bottom:1px solid #F3F4F6;display:flex;align-items:center;gap:10px">
      ...
      <div style="text-align:right;flex-shrink:0">
        ...
        <button class="btn btn-purple" style="padding:5px 12px;font-size:11px;margin-top:4px" onclick="bookLabTest('${l.id}','${t.id}','${safe(t.test_name).replace(/'/g,"\\'")}')">Book</button>
      </div>
    </div>`).join("")}
...
async function refreshUserData(role,userId){
  ...
  try{
    ...
    if(rows.length){
      A.user=rows[0];S.save(rows[0],role);
      toast("DEBUG: is_available = "+rows[0].is_available,false);
    }
  }catch(e){toast("REFRESH ERROR: "+e.message,true);}
}
