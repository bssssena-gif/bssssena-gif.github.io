...
async function searchDonors(){
  if(!A.user||A.role!=="patient"){
    ...
    if(res)res.innerHTML=`<div style="text-align:center;padding:30px;background:#FFF5F5;border-radius:12px;border:1px solid #FED7D7">
      ...
      <div style="font-weight:700;color:#B23434;margin-bottom:6px">Login Required</div>
      <div style="font-size:13px;color:#6B7280;margin-bottom:12px">Donor details చూడటానికి login చేయండి</div>
      <button class="btn btn-primary btn-sm" onclick="go('patientLogin')" style="background:#B23434">Login చేయండి</button>
    </div>`;
    return;
  }
  ...
  try{
    ...
    res.innerHTML=donors.map(d=>`
      <div class="appt-card" style="border-left:4px solid #B23434;margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px">
          <div>
            <div style="font-size:14px;font-weight:700;color:#122A4A">${safe(d.name)}</div>
          ...
          <div style="text-align:center">
            <div style="background:#B23434;color:#fff;border-radius:8px;padding:6px 14px;font-size:18px;font-weight:800">${safe(d.blood_group)}</div>
          </div>
        </div>
        ${d.phone?`<button class="btn btn-primary btn-sm" style="margin-top:10px;width:100%;background:#B23434" onclick="callWithLogin('${safe(d.phone)}')">📞 Contact Donor</button>`:""}
      </div>`).join("");
  }catch(e){res.innerHTML=`<div style="color:#B23434;padding:16px">Error: ${e.message}</div>`;}
}
...
async function sendSymptom(){
  ...
  chat.innerHTML+=`<div style="background:#0F3D73;color:#fff;border-radius:10px;padding:12px;font-size:13px;align-self:flex-end;max-width:85%">${safe(text)}</div>`;

  // Loading bubble
  const loadId="ai-loading-"+Date.now();
  chat.innerHTML+=`<div id="${loadId}" style="background:#E3F4FC;border-radius:10px;padding:12px;font-size:13px;color:#122A4A;align-self:flex-start;max-width:85%">🤔 Analyzing symptoms...</div>`;
  ...
  if(bpCtx){
    ...
    hdCharts.bp=new Chart(bpCtx,{
      ...
      data:{
        labels,
        datasets:[
          {label:"Systolic",data:reversed.map(v=>v.bp_systolic),borderColor:"#B23434",backgroundColor:"rgba(178,52,52,0.08)",tension:0.3,pointRadius:4},
          {label:"Diastolic",data:reversed.map(v=>v.bp_diastolic),borderColor:"#0F3D73",backgroundColor:"rgba(15,61,115,0.08)",tension:0.3,pointRadius:4}
        ]
      },
    ...
    hdCharts.sugar=new Chart(sgCtx,{
      ...
      data:{
        ...
        datasets:[
          ...
          {label:"Post-Prandial",data:reversed.map(v=>v.sugar_pp),borderColor:"#1C8F7D",backgroundColor:"rgba(28,143,125,0.08)",tension:0.3,pointRadius:4}
        ]
      },
...
function renderVitalsTable(vitals){
  ...
  el.innerHTML=`<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12px">
    ...
    <tbody>${vitals.map(v=>`<tr style="border-bottom:1px solid #F3F4F6">
      ...
      <td style="padding:8px;text-align:center">${v.bp_systolic&&v.bp_diastolic?`<span style="color:#B23434;font-weight:600">${v.bp_systolic}/${v.bp_diastolic}</span>`:"—"}</td>
...
async function loadHomeProviders(){
  ...
  }catch(e){el.innerHTML=`<div style="color:#B23434;padding:12px">Error: ${e.message}</div>`;}
}

function hpCard(p){
  const typeLabel=p.service_type==="nursing"?"👩‍⚕️ Home Nursing":p.service_type==="physiotherapy"?"🦽 Physiotherapy":"👩‍⚕️🦽 Nursing & Physio";
  const typeColor=p.service_type==="nursing"?"#0891B2":p.service_type==="physiotherapy"?"#1C8F7D":"#5B4FCF";
    ...
    <div style="display:flex;gap:12px;margin-bottom:12px">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="display:flex;gap:6px;margin-top:3px;flex-wrap:wrap;align-items:center">
          ...
          <span style="font-size:10px;font-weight:600;color:#1C8F7D">🟢 Available</span>
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:#1C8F7D">${p.fee?"₹"+p.fee+"/visit":"Free"}</div>
  ...
  if(sel){
    ...
    sel.innerHTML=`<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:10px;padding:12px;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div>
          <div style="font-weight:700;color:#122A4A">${name}</div>
...
function selectHSTab(type){
  ...
  if(type==="nursing"){
    ...
    if(physio){physio.style.borderColor="#1C8F7D";physio.style.background="#DCFCE7";}
...
async function loadMyHomeServices(){
  ...
  try{
    ...
    const statusColor={pending:"#D97706",confirmed:"#1C8F7D",completed:"#6B7280",cancelled:"#B23434"};
    ...
    el.innerHTML=bookings.map(b=>`
      <div class="appt-card" style="border-left:4px solid ${statusColor[b.status]||"#6B7280"};margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:6px">
          <div>
            <div style="font-size:14px;font-weight:700;color:#122A4A">${b.service_type==="nursing"?"👩‍⚕️ Home Nursing":"🦽 Physiotherapy"}</div>
...
async function rHPDash(){
  ...
  if(avBtn){
    ...
    avBtn.style.background=A.user.is_available?"rgba(255,255,255,.2)":"rgba(214,69,69,.4)";
  }
...
function renderHPBookings(){
  ...
  el.innerHTML=bookings.map(b=>`
    <div class="appt-card" style="border-left:4px solid ${statusColor[b.status]||"#6B7280"};margin-bottom:12px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:6px">
        <div>
          <div style="font-size:14px;font-weight:700;color:#122A4A">${b.service_type==="nursing"?"👩‍⚕️ Nursing":"🦽 Physiotherapy"}</div>
      ...
      <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
        ${b.status==="pending"?`<button onclick="hpUpdateBooking('${b.id}','confirmed')" class="btn btn-sm" style="background:#D1FAE5;color:#1C8F7D;border:none;padding:6px 14px;border-radius:8px;cursor:pointer;font-weight:600">✅ Accept</button>`:""}
...
async function toggleHPAvail(){
  ...
  try{
    ...
    const avBtn=$("hp-avail-btn");
    if(avBtn){avBtn.textContent=next?"🟢 Available":"🔴 Offline";avBtn.style.background=next?"rgba(255,255,255,.2)":"rgba(214,69,69,.4)";}
...
function hospitalCard(h){
  ...
  const availColor=h.beds_available>10?"#1C8F7D":h.beds_available>0?"#D97706":"#B23434";
  return `<div class="card" style="margin-bottom:12px">
    <div style="display:flex;gap:12px;margin-bottom:10px">
      <div style="width:54px;height:54px;border-radius:12px;background:#B23434;display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0">🏥</div>
      <div style="flex:1;min-width:0">
        <div style="font-size:15px;font-weight:700;color:#122A4A">${safe(h.name)}</div>
        <div style="font-size:12px;color:#B23434;font-weight:600">${typeLabel[h.hospital_type]||"🏥 Hospital"}</div>
        <div style="font-size:12px;color:#6B7280;margin-top:2px">📍 ${safe(h.district)}</div>
        ${h.emergency_available?`<span style="background:#FEF2F2;color:#B23434;border-radius:20px;padding:2px 8px;font-size:10px;font-weight:700">🚨 24×7 Emergency</span>`:""}
      </div>
      <div style="text-align:right;flex-shrink:0">
    ...
    <div style="display:flex;gap:8px">
      ...
      ${h.phone?`<button class="btn btn-primary btn-sm" style="background:#B23434;flex:1" onclick="callWithLogin('${safe(h.phone)}')">📞 Call</button>`:""}
    </div>
  </div>`;
}
...
function setPostType(type){
  ...
  ["tip","awareness","news"].forEach(t=>{
    const btn=$("pt-"+t);
    if(btn){btn.style.background=t===type?"#5B4FCF":"";btn.style.color=t===type?"#fff":"";}
  });
}
...
function filterFeed(filter){
  ...
  ["all","tip","awareness","news"].forEach(f=>{
    const btn=$("ff-"+f);
    if(btn){btn.style.background=f===filter?"#5B4FCF":"";btn.style.color=f===filter?"#fff":"";}
  });
  loadFeed();
}
...
function postCard(p){
  ...
  const typeColor={tip:"#1C8F7D",awareness:"#5B4FCF",news:"#D97706",text:"#6B7280"};
  ...
  return `<div style="background:#fff;border-radius:14px;padding:16px;box-shadow:0 2px 8px rgba(0,0,0,.06)">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
      <div style="width:40px;height:40px;border-radius:50%;background:#5B4FCF;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:16px;flex-shrink:0">${safe(p.author_name||"U").charAt(0).toUpperCase()}</div>
      <div style="flex:1">
        <div style="font-weight:700;font-size:14px;color:#122A4A">${roleIcon[p.author_role]||"👤"} ${safe(p.author_name)}</div>
    ...
    <div style="font-size:14px;color:#122A4A;line-height:1.6;margin-bottom:12px">${safe(p.content)}</div>
    ...
    <div id="comments-${p.id}" style="display:none;margin-top:10px;border-top:1px solid #F3F4F6;padding-top:10px">
      ...
      ${A.user?`<div style="display:flex;gap:8px;margin-top:8px">
        ...
        <button onclick="addComment('${p.id}')" class="btn btn-primary btn-sm" style="background:#5B4FCF">Send</button>
      </div>`:""}
    </div>
  </div>`;
}
...
async function loadComments(postId){
  ...
  try{
    ...
    el.innerHTML=comments.map(c=>`
      <div style="display:flex;gap:8px;margin-bottom:8px;align-items:flex-start">
        <div style="width:28px;height:28px;border-radius:50%;background:#5B4FCF;display:flex;align-items:center;justify-content:center;color:#fff;font-size:11px;font-weight:700;flex-shrink:0">${safe(c.author_name).charAt(0).toUpperCase()}</div>
        <div style="background:#F8FAFC;border-radius:10px;padding:8px 10px;flex:1">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div style="font-size:11px;font-weight:700;color:#122A4A">${safe(c.author_name)}</div>
  ...
  }catch(e){el.innerHTML=`<div style="color:#B23434;font-size:12px">Error loading comments</div>`;}
}
...
async function loadMyPosts(role){
  ...
  try{
    ...
    el.innerHTML=posts.map(p=>postCard(p)).join("");
  }catch(e){el.innerHTML=`<div style="color:#B23434">Error loading posts</div>`;}
}
...
function setDocPostType(type){
  ...
  ["tip","awareness","news"].forEach(t=>{
    const btn=$("dpt-"+t);
    if(btn){btn.style.background=t===type?"#0F3D73":"";btn.style.color=t===type?"#fff":"";}
  });
}
...
function shareProfile(type, id, name, subtype, district){
  ...
  if(navigator.share){
    // Native share (Android/iOS) — url intentionally omitted since it's already inside `text`
    // (passing both `text` and `url` makes apps like WhatsApp show the link/info twice)
    navigator.share({
      title:"DocNear — "+name,
      text:text
    }).catch(()=>{});
...
function renderReviewForm(docId){
  ...
  <div style="background:#fff;border-radius:16px;padding:20px;box-shadow:0 2px 8px rgba(0,0,0,.05);margin-top:14px">
    <div style="font-size:14px;font-weight:700;color:#122A4A;margin-bottom:12px">Write a Review</div>
...
async function rAdvancedSearch(){
  ...
  try{
    ...
    // Client-side filters
    if(A.q){const v=A.q.trim().toLowerCase();docs=docs.filter(d=>
    ...
    if(list){
      list.innerHTML=notifs.length?notifs.map(n=>`
        <div style="padding:10px;border-bottom:1px solid #F3F4F6;cursor:pointer" onclick="markNotifRead('${n.id}');this.style.opacity='0.5'">
          <div style="font-size:13px;font-weight:600;color:#122A4A">${safe(n.title)}</div>
