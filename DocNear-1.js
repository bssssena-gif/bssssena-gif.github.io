...
/* ── Specializations ── */
const SPECS=[
  {n:"Cardiology",i:"❤️",c:"#FF4757"},{n:"Neurology",i:"🧠",c:"#5B4FCF"},
  {n:"Orthopedics",i:"🦴",c:"#FFC22E"},{n:"Pediatrics",i:"👶",c:"#00BCD4"},
...
const DEMO={
  docs:[
    ...
    {id:"d2",name:"Dr. Rajesh Kumar",email:"rajesh@docnear.com",phone:"+91 98765 12345",specialization:"Neurology",reg_number:"MCI-2017-32145",hospital:"KIMS Hospital",city:"Hyderabad",district:"Hyderabad",state_code:"TS",experience:15,fee:1200,rating:4.9,reviews:312,approved:true,color:"#5B4FCF",slots:["10:00 AM","11:00 AM","3:00 PM","4:00 PM"],video_consult:true,about:"Senior Neurologist specializing in epilepsy and stroke.",qualifications:"MBBS, DM (Neurology)",subscription:"yearly"},
  ...
  stores:[
    {id:"s1",store_name:"Apollo Pharmacy",owner_name:"Ravi Kumar",email:"apollo@store.com",phone:"+91 98765 11111",district:"Hyderabad",state_code:"TS",address:"Banjara Hills, Hyderabad",is_24x7:true,delivery_available:true,delivery_radius_km:10,approved:true,color:"#22A490",rating:4.5,opening_time:"00:00",closing_time:"23:59"},
    {id:"s2",store_name:"MedPlus Pharmacy",owner_name:"Suresh Reddy",email:"medplus@store.com",phone:"+91 98765 22222",district:"Hyderabad",state_code:"TS",address:"Jubilee Hills, Hyderabad",is_24x7:false,delivery_available:true,delivery_radius_km:5,approved:true,color:"#1C8F7D",rating:4.3,opening_time:"08:00",closing_time:"22:00"}
  ],
  ambus:[
    {id:"a1",operator_name:"Srinivas Rao",email:"ambu1@docnear.com",phone:"+91 99001 11111",alternate_phone:"+91 99001 11112",district:"Hyderabad",state_code:"TS",base_location:"Jubilee Hills",vehicle_type:"Advanced Life Support",vehicle_number:"TS 09 AB 1234",base_fare:500,per_km_rate:20,night_charge_extra:200,service_radius_km:25,has_oxygen:true,has_stretcher:true,has_monitor:true,approved:true,availability_status:"available",color:"#D64545",rating:4.8,subscription:"pro"},
    {id:"a2",operator_name:"Ramesh Kumar",email:"ambu2@docnear.com",phone:"+91 99002 22222",alternate_phone:"+91 99002 22223",district:"Hyderabad",state_code:"TS",base_location:"Secunderabad",vehicle_type:"Basic Life Support",vehicle_number:"TS 09 CD 5678",base_fare:350,per_km_rate:15,night_charge_extra:150,service_radius_km:30,has_oxygen:true,has_stretcher:true,has_monitor:false,approved:true,availability_status:"available",color:"#B23434",rating:4.5,subscription:"basic"}
  ],
  labs:[
    {id:"l1",lab_name:"Vijaya Diagnostics",owner_name:"Dr. Vijay Kumar",email:"vijaya@lab.com",phone:"+91 98111 11111",district:"Hyderabad",state_code:"TS",address:"Banjara Hills",lab_type:"full_service",nabl_accredited:true,home_collection:true,home_collection_charge:150,approved:true,color:"#5B4FCF",rating:4.7,subscription:"growth"},
...
function avt(n,c="#0F3D73",sz=48,photoUrl=""){
  if(photoUrl&&photoUrl.startsWith("http")){
    return `<div style="width:${sz}px;height:${sz}px;border-radius:50%;overflow:hidden;flex-shrink:0;background:${c}">
      <img src="${photoUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:50%" class="provider-avatar"
        onerror="this.parentElement.innerHTML='<div style=\"width:${sz}px;height:${sz}px;display:flex;align-items:center;justify-content:center;font-size:${Math.round(sz*.35)}px;font-weight:700;color:#fff;background:${c}\">${ini(n)}</div>'" />
    </div>`;
  }
  return `<div class="avatar" style="width:${sz}px;height:${sz}px;background:${c};font-size:${Math.round(sz*.35)}px">${ini(n)}</div>`;
}
function pill(l,bg="#E3F4FC",c="#0F3D73"){
  return `<span class="pill" style="background:${bg};color:${c}">${l}</span>`;
}
function sPill(s){
  const m={confirmed:["#D1FAE5","#1C8F7D"],pending:["#FEF3C7","#D97706"],cancelled:["#FEE2E2","#B23434"],completed:["#EDE9FE","#5B4FCF"],available:["#D1FAE5","#1C8F7D"],busy:["#FEF3C7","#D97706"],offline:["#F3F4F6","#9CA3AF"]};
  const[bg,c]=m[s]||m.pending;return pill(s.toUpperCase(),bg,c);
}
function toast(msg,err=false){
  const el=$("toast");if(!el)return;
  el.style.background=err?"#D64545":"#22A490";el.style.color="#fff";
...
function sc(ic,lb,v,bg="#E3F4FC",vc="#0F3D73",scrollTo=""){
...
async function uploadProfilePhoto(file, role, userId){
  if(!file)return null;
  if(file.size > 5*1024*1024){toast("Photo 5MB కంటే తక్కువగా ఉండాలి",true);return null;}
  
  try{
    // Compress to small size
    const compressed = await compressImage(file, 300, 300, 0.75);
    const ts = Date.now();
    const fileName = `${role}_${userId}_${ts}.jpg`;
    ...
    if(!res.ok){
      ...
      // Bucket public కాకపోతే — different approach try
      if(res.status===400||res.status===403){
        toast("Storage error: Supabase లో profiles bucket public చేయండి",true);
      }else{
        toast("Upload failed ("+res.status+"): "+errText.slice(0,50),true);
      }
      return null;
    }
    
    const publicUrl = `${SURL}/storage/v1/object/public/profiles/${fileName}`;
...
function showPhotoUpload(role, userId, currentPhotoUrl){
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:24px;max-width:340px;width:100%">
      <div style="font-size:16px;font-weight:700;color:#122A4A;margin-bottom:16px">📷 Profile Photo</div>
      <div style="text-align:center;margin-bottom:16px">
        <div id="photo-preview-wrap" style="width:100px;height:100px;border-radius:50%;overflow:hidden;margin:0 auto;background:#F3F4F6;border:3px solid #E5E7EB;display:flex;align-items:center;justify-content:center">
          ${currentPhotoUrl ? `<img src="${currentPhotoUrl}" style="width:100%;height:100%;object-fit:cover"/>` : '<div style="font-size:36px">👤</div>'}
        </div>
        <div style="font-size:11px;color:#9CA3AF;margin-top:8px">Tap below to choose photo</div>
      </div>
      <label style="display:block;background:#F8FAFC;border:2px dashed #E5E7EB;border-radius:12px;padding:14px;text-align:center;cursor:pointer;margin-bottom:12px">
        <div style="font-size:24px;margin-bottom:4px">📁</div>
        <div style="font-size:13px;color:#0F3D73;font-weight:600">Photo Select చేయండి</div>
        <div style="font-size:11px;color:#9CA3AF">JPG, PNG, WebP • Max 5MB</div>
    ...
    if(url){
      ...
      if(table){
        try{
          ...
          console.error("Photo DB save error:", dbErr);
          toast("Photo save అవ్వలేదు: "+(dbErr?.message||"Unknown error"),true);
        }
      }
      // Update dashboard photo immediately (even if DB save failed, show it for this session)
      const imgs = document.querySelectorAll('.provider-avatar');
      imgs.forEach(img => {
        img.src = url;
        img.style.display = 'block';
        if(img.nextElementSibling) img.nextElementSibling.style.display = 'none';
      });
      if(saved)toast("Photo uploaded successfully! ✅");
      // Re-render dashboard to show photo in cards
      if(A.role==="doctor") rDoctorDash();
      else if(A.role==="store") rStoreDash();
      else if(A.role==="ambulance") rAmbuDash();
      else if(A.role==="lab") rLabDash();
      else if(A.role==="homeprovider") rHPDash();
      else if(A.role==="hospital") rHospitalDash();
      // Close modal
      const modal = btn.closest('div[style*=fixed]');
      if(modal) modal.remove();
    }
  }catch(e){
    console.error("Photo upload UI error:", e);
    toast("Photo update సమస్య: "+(e?.message||"Unknown")+" — మళ్ళీ ప్రయత్నించండి",true);
  }finally{
...
/* Provider dashboard photo load */
function loadProviderPhoto(photoUrl){
  if(!photoUrl) return;
  const imgs = document.querySelectorAll('.provider-avatar');
  imgs.forEach(img => {
    img.src = photoUrl;
...
function openSupportModal(){
  ...
  if(A.user){
    const n=$("sup-name"),e=$("sup-email");
    if(n&&!n.value)n.value=A.user.name||"";
    if(e&&!e.value)e.value=A.user.email||A.user.phone||"";
  }
}
async function submitSupportMsg(){
  const name=gv("sup-name"),email=gv("sup-email"),subject=gv("sup-subject"),message=gv("sup-message");
  if(!name||!email||!message){toast("పేరు, Email/Phone, Message అవసరం",true);return;}
  const btn=$("sup-submit-btn");
  if(btn){btn.disabled=true;btn.textContent="పంపుతోంది...";}
  try{
    await DB.post("support_messages",{name,email,subject:subject||null,message,status:"open"});
    toast("మీ మెసేజ్ పంపబడింది! త్వరలో సంప్రదిస్తాము ✅");
    const modal=$("support-modal");if(modal)modal.style.display="none";
    ["sup-name","sup-email","sup-subject","sup-message"].forEach(id=>{const el=$(id);if(el)el.value="";});
  }catch(e){
    toast("పంపడంలో సమస్య వచ్చింది, మళ్ళీ ప్రయత్నించండి",true);
  }finally{
    if(btn){btn.disabled=false;btn.textContent="పంపండి";}
  }
}
...
function setLang(lang){
  ...
  [$("btn-lang-en"),$("btn-lang-te")].forEach((b,i)=>{
    ...
    b.style.background=on?"linear-gradient(135deg,#0F3D73,#0A2C54)":"transparent";
  ...
  return `<div class="form-group">
    ...
    <div id="plan-cards-${role}" style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:4px">
      ...
      <div class="plan-card ${p.popular?"popular":""}" id="plan-${role}-${p.id}" onclick="selectPlan('${role}','${p.id}')" style="position:relative">
        ${p.popular?`<div style="position:absolute;top:-10px;right:12px;background:#FFC22E;color:#fff;font-size:10px;font-weight:700;padding:2px 10px;border-radius:12px">⭐ POPULAR</div>`:""}
        <div style="font-size:13px;font-weight:700;color:#122A4A;margin-bottom:6px">${p.name}</div>
...
async function doctorRegister(){
  ...
  try{
    ...
    await safePost("doctors",{
      ...
      approved:false,color:SPECS.find(s=>s.n===f.spec)?.c||"#0F3D73",
...
async function storeRegister(){
  ...
  try{
    ...
    await safePost("medical_stores",{
      ...
      approved:false,is_active:true,color:"#22A490",delivery_available:false,is_24x7:false
    });
...
async function ambuRegister(){
  ...
  try{
    ...
    await safePost("ambulances",{
      ...
      base_location:f.loc,
      approved:false,availability_status:"offline",color:"#D64545",
...
async function labRegister(){
  ...
  try{
    ...
    await safePost("diagnostic_labs",{
      ...
      approved:false,is_active:true,color:"#5B4FCF",
  ...
  modal.innerHTML=`
    <div style="background:#fff;border-radius:20px;padding:28px 24px;max-width:420px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.3)">
      <div style="text-align:center;margin-bottom:20px">
        ...
        <h3 style="font-family:'Sora',serif;font-size:20px;color:#122A4A;margin-bottom:4px">Complete Payment</h3>
      ...
      <div style="background:#F0F9FF;border-radius:14px;padding:16px;margin-bottom:20px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <div style="font-size:14px;font-weight:600;color:#122A4A">${plan.name} Plan</div>
          <div style="font-size:20px;font-weight:800;color:#0F3D73">₹${plan.price}<span style="font-size:12px;color:#9CA3AF">${plan.period}</span></div>
        </div>
      ...
      <div id="pay-err" style="display:none;background:#FEE2E2;color:#B23434;padding:8px 12px;border-radius:8px;font-size:12px;margin-bottom:10px"></div>
...
function selectPayMethod(m){
  ["upi","card","nb"].forEach(id=>{
    const el=$("pay-"+id);
    if(el)el.style.borderColor=id===m||(id==="nb"&&m==="netbanking")?"#0F3D73":"transparent";
  });
  window._payMethod=m;
}
    ...
    safeGet("medical_stores","approved=eq.true&limit=3","stores").then(stores=>{
      ...
      stc.innerHTML=stores.length?stores.map(s=>`<div class="card" onclick="go('stores')">
        <div style="display:flex;gap:12px;margin-bottom:10px">
          ${avt(s.store_name||s.name,"#22A490",48,s.photo_url||"")}
          <div style="flex:1"><div style="font-size:14px;font-weight:700;color:#122A4A">${safe(s.store_name||s.name)}</div>
          <div style="font-size:12px;color:#1C8F7D">💊 Medical Store</div>
          <div style="font-size:11px;color:#6B7280">📍 ${safe(s.district||s.city||"—")}</div>
          <div style="font-size:11px;color:${s.is_available!==false?"#1C8F7D":"#B23434"};font-weight:600">${s.is_available!==false?"🟢 Open":"🔴 Closed"}</div></div>
        </div>
  ...
  if(amc){
    ...
    safeGet("ambulances","approved=eq.true&availability_status=eq.available&limit=3","ambus").then(ambus=>{
      ...
      amc.innerHTML=ambus.length?ambus.map(a=>`<div class="card" onclick="go('ambulance')">
        <div style="display:flex;gap:12px;margin-bottom:10px">
          ${avt(a.operator_name,"#D64545",48,a.photo_url||"")}
          <div style="flex:1"><div style="font-size:14px;font-weight:700;color:#122A4A">${safe(a.operator_name)}</div>
          <div style="font-size:12px;color:#D64545">🚑 ${safe(a.vehicle_type||"Ambulance")}</div>
          <div style="font-size:11px;color:#6B7280">📍 ${safe(a.base_location||a.district||"—")}</div>
          <div style="font-size:11px;color:#1C8F7D;font-weight:600">🟢 Available</div></div>
        </div>
        ${A.user?`<div style="font-size:12px;color:#6B7280;margin-bottom:8px">📞 ${safe(a.phone||"—")}</div>`:""}
        <div style="display:flex;gap:6px">
          ...
          <button class="btn btn-primary" style="flex:1;font-size:12px;padding:7px;background:#D64545;border-color:#D64545" onclick="event.stopPropagation();callWithLogin('${safe(a.phone)}')">📞 Call Now</button>
        </div>
  ...
  if(lbc){
    ...
    safeGet("diagnostic_labs","approved=eq.true&limit=3","labs").then(labs=>{
      ...
      lbc.innerHTML=labs.length?labs.map(l=>`<div class="card" onclick="go('labsPage')">
        <div style="display:flex;gap:12px;margin-bottom:10px">
          ${avt(l.lab_name||l.name,"#5B4FCF",48,l.photo_url||"")}
          <div style="flex:1"><div style="font-size:14px;font-weight:700;color:#122A4A">${safe(l.lab_name||l.name)}</div>
          <div style="font-size:12px;color:#5B4FCF">🔬 Diagnostic Lab</div>
          <div style="font-size:11px;color:#6B7280">📍 ${safe(l.district||l.city||"—")}</div>
          <div style="font-size:11px;color:${l.is_available!==false?"#1C8F7D":"#B23434"};font-weight:600">${l.is_available!==false?"🟢 Open":"🔴 Closed"}</div></div>
        </div>
        <div style="display:flex;gap:6px">
          ...
          <button class="btn btn-primary" style="flex:1;font-size:12px;padding:7px;background:#5B4FCF;border-color:#5B4FCF" onclick="event.stopPropagation();viewLab('${l.id}')">🔬 View & Book</button>
        </div>
    ...
    dbq("health_posts?order=created_at.desc&limit=3").then(posts=>{
      lf.innerHTML=posts.length?posts.map(p=>`<div style="background:#fff;border-radius:12px;padding:14px;box-shadow:0 2px 8px rgba(0,0,0,.06)">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
          <div style="width:34px;height:34px;border-radius:50%;background:#5B4FCF;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:14px">${safe(p.author_name||"U").charAt(0).toUpperCase()}</div>
          <div><div style="font-weight:700;font-size:13px;color:#122A4A">${safe(p.author_name)}</div><div style="font-size:11px;color:#9CA3AF">${timeAgo(p.created_at)}</div></div>
        </div>
...
function docCard(d){
  const sc=(SPECS.find(s=>s.n===d.specialization)||{}).c||"#0F3D73";
  const isFav=A.cache.favs?.includes(d.id);
  const sub=d.subscription==="yearly"?pill("⭐ Premium","#FEF3C7","#D97706"):d.subscription==="monthly"?pill("✓ Listed","#D1FAE5","#1C8F7D"):"";
  return `<div class="card" onclick="A.user?viewDoc('${d.id}'):requireLogin('search')">
    <div style="display:flex;gap:12px;margin-bottom:12px">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="display:flex;gap:4px;margin-top:3px;flex-wrap:wrap;align-items:center">${sub}${d.district?`<span style="font-size:10px;color:#9CA3AF">📍 ${d.district}</span>`:""}
          <span style="font-size:10px;font-weight:600;color:${d.is_available!==false?"#1C8F7D":"#B23434"}">${d.is_available!==false?"🟢 Available":"🔴 Offline"}</span>
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:13px;font-weight:600;color:#1C8F7D">Free Consultation</div>
...
function apptCard(a,forDoc=false){
  const bc={confirmed:"#1C8F7D",pending:"#D97706",cancelled:"#B23434",completed:"#5B4FCF"}[a.status]||"#D97706";
  return `<div class="appt-card" style="border-left:4px solid ${bc}">
    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px">
      <div><div style="font-size:14px;font-weight:700;color:#122A4A">${forDoc?safe(a.patient_name):safe(a.doctor_name)}</div>
      <div style="font-size:12px;color:#6B7280">${safe(a.specialization)}</div>
      ${forDoc&&a.patient_phone?`<div style="font-size:12px;color:#0F3D73;font-weight:600">📞 ${safe(a.patient_phone)}</div>`:""}
      </div>
      ${sPill(a.status)}
    </div>
    <div style="display:flex;gap:12px;margin-top:8px;flex-wrap:wrap;align-items:center">
      ...
      ${a.is_video?`<span style="font-size:11px;color:#5B4FCF;font-weight:600">🎥 Video</span>`:""}
    </div>
    ${a.payment_status?`<div style="margin-top:6px">${pill(a.payment_status==="paid"?"💳 Paid":"⏳ Unpaid",a.payment_status==="paid"?"#D1FAE5":"#FEF3C7",a.payment_status==="paid"?"#1C8F7D":"#D97706")}</div>`:""}
...
async function rSearch(){
  ...
  if(na)na.innerHTML=A.user?`<span style="font-size:12px;font-weight:600;color:#122A4A">${A.user.name||A.user.store_name||"User"}</span><button class="btn btn-ghost btn-sm" onclick="logout()">Logout</button>`:`<button class="btn btn-outline btn-sm" onclick="go('patientLogin')">Login</button>`;
...
async function filterDoctors(){A.q=(($("search-q")||{}).value||"").trim();A.spec=($("search-spec")||{}).value||"";document.querySelectorAll(".chip").forEach(c=>{const isAll=c.textContent.trim().startsWith("All");c.classList.toggle("active",isAll?!A.spec:(!!A.spec&&c.textContent.includes(A.spec)));});await fetchDocs();}
async function fetchDocs(){
  ...
  try{
    ...
    if(A.q){const v=A.q.trim().toLowerCase();docs=docs.filter(d=>safe(d.name).toLowerCase().includes(v)||safe(d.specialization).toLowerCase().includes(v)||safe(d.hospital).toLowerCase().includes(v)||safe(d.city).toLowerCase().includes(v)||safe(d.location).toLowerCase().includes(v)||safe(d.district).toLowerCase().includes(v));}
...
async function rPatientDash(){
  ...
  try{
    ...
    if($("pd-stats"))$("pd-stats").innerHTML=
      sc("📅","Appointments",A.cache.appts.length,"#E3F4FC","#0F3D73")+
      sc("✅","Confirmed",A.cache.appts.filter(a=>a.status==="confirmed").length,"#D1FAE5","#1C8F7D")+
      sc("🎥","Video",A.cache.appts.filter(a=>a.is_video).length,"#EDE9FE","#5B4FCF")+
      sc("👨‍⚕️","Doctors",new Set(A.cache.appts.map(a=>a.doctor_id)).size,"#FEE9E1","#FFC22E");
    ...
    if($("pd-lab-bookings"))$("pd-lab-bookings").innerHTML=labBookings.length?labBookings.map(b=>`<div class="appt-card" style="border-left:4px solid #5B4FCF">
      <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px">
    ...
    if($("pd-orders"))$("pd-orders").innerHTML=orders.length?orders.map(o=>`<div class="appt-card" style="border-left:4px solid #22A490">
      <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px">
...
async function rDoctorDash(){
  ...
  try{
    ...
    if($("dd-stats"))$("dd-stats").innerHTML=
      sc("📅","Total",appts.length)+sc("🌅","Today",todayA.length,"#FEE9E1","#FFC22E")+
      sc("💰","Revenue","₹"+rev.toLocaleString(),"#EDE9FE","#5B4FCF")+
      sc("✅","Confirmed",appts.filter(a=>a.status==="confirmed").length,"#D1FAE5","#1C8F7D");
    ...
    if($("dd-status-pill"))$("dd-status-pill").innerHTML=pill(u.approved?"✓ Approved":"⏳ Pending",u.approved?"#D1FAE5":"#FEF3C7",u.approved?"#1C8F7D":"#D97706");
  if($("dd-avail-btn")){
    const av=u.is_available!==false;
    $("dd-avail-btn").textContent=av?"🟢 Available — Click to go Offline":"🔴 Offline — Click to go Online";
    $("dd-avail-btn").style.background=av?"#1C8F7D":"#B23434";
  }
    ...
    if($("dd-video-btn")){const vc=u.video_consult||false;$("dd-video-btn").textContent=vc?"🎥 Video ON — Disable":"🎥 Video OFF — Enable";$("dd-video-btn").style.background=vc?"#5B4FCF":"#9CA3AF";}
...
async function rStoreDash(){
  ...
  try{
    ...
    if($("sd-stats"))$("sd-stats").innerHTML=
      ...
      sc("✅","Completed",orders.filter(o=>o.status==="completed").length,"#D1FAE5","#1C8F7D")+
      sc("💰","Revenue","₹"+orders.filter(o=>o.payment_status==="paid").reduce((s,o)=>s+(o.total_amount||0),0).toLocaleString(),"#EDE9FE","#5B4FCF");
    if($("sd-orders"))$("sd-orders").innerHTML=orders.length
      ?orders.map(o=>`<div class="appt-card" style="border-left:4px solid ${{"pending":"#D97706","accepted":"#0F3D73","packed":"#5B4FCF","delivered":"#1C8F7D","completed":"#1C8F7D","rejected":"#B23434"}[o.status]||"#9CA3AF"}">
        ...
        ${o.is_urgent?`<span style="font-size:11px;background:#FEE2E2;color:#B23434;padding:2px 8px;border-radius:6px;font-weight:600">🚨 URGENT</span>`:""}
    ...
    $("sd-avail-btn").style.background=av?"#1C8F7D":"#B23434";
  }
...
async function rAmbuDash(){
  ...
  if($("amd-stats"))$("amd-stats").innerHTML=
    ...
    sc("📍","Location",safe(u.base_location||u.district)||"—","#FEE9E1","#FFC22E")+
    sc("📞","Phone",safe(u.phone)||"—","#EDE9FE","#5B4FCF")+
    sc("🔴","Status",u.availability_status==="available"?"🟢 Online":"🔴 Offline","#D1FAE5","#1C8F7D");
  if($("amd-avail-btn")){
    ...
    $("amd-avail-btn").style.background=av==="available"?"#1C8F7D":av==="busy"?"#D97706":"#B23434";
  }
  ...
  ].map(([ic,n,has])=>`<span style="padding:6px 12px;border-radius:8px;font-size:12px;font-weight:600;background:${has?"#D1FAE5":"#F3F4F6"};color:${has?"#1C8F7D":"#9CA3AF"}">${ic} ${n}: ${has?"✓":"✗"}</span>`).join("");
}
...
async function rLabDash(){
  ...
  if($("ld-avail-btn")){
    ...
    $("ld-avail-btn").style.background=av?"#1C8F7D":"#B23434";
  }
  ...
  try{
    ...
    if($("ld-stats"))$("ld-stats").innerHTML=
      ...
      sc("✅","Completed",bk.filter(b=>b.status==="completed").length,"#D1FAE5","#1C8F7D")+
      sc("🧪","Tests",tests.length,"#EDE9FE","#5B4FCF");
    if($("ld-bookings"))$("ld-bookings").innerHTML=bk.length?bk.map(b=>`<div class="appt-card" style="border-left:4px solid #5B4FCF">
      <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px">
        <div><div style="font-size:14px;font-weight:700">${safe(b.patient_name)||"Patient"}</div>
        <div style="font-size:12px;color:#0F3D73;font-weight:600">📞 ${safe(b.patient_phone)||"—"}</div>
