...
function showPhotoUpload(role, userId, currentPhotoUrl){
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:24px;max-width:340px;width:100%">
      <div style="font-size:16px;font-weight:700;color:#1A2B3C;margin-bottom:16px">📷 Profile Photo</div>
      ...
      <label style="display:block;background:#F8FAFC;border:2px dashed #E5E7EB;border-radius:12px;padding:14px;text-align:center;cursor:pointer;margin-bottom:12px">
        ...
        <div style="font-size:13px;color:#0A7FAF;font-weight:600">Photo Select చేయండి</div>
...
function redirectRole(role){
  ...
  if(role==="admin"){setTimeout(()=>{window.open("admin.html","_blank")||(window.location.href="admin.html");},800);return;}
  go(r[role]||"landing");
}
...
async function adminLogin(){
  ...
  const adminTab=window.open("","_blank"); // open blank tab NOW (sync, so popup blockers allow it)
  try{
    let ok=false;
    const hash=await sha(pw);
    ...
    if(!ok){toast("Invalid admin credentials.",true);if(adminTab)adminTab.close();return;}
    A.role="admin";S.save(A.user,"admin");
toast("Welcome, Admin! 🛡️");
await new Promise(r=>setTimeout(r,500));
if(adminTab){adminTab.location.href="admin.html";}else{window.location.href="admin.html";}
    
  }catch(e){toast(e.message,true);if(adminTab)adminTab.close();}
